...
# CSV Header
echo "Repository,Created Date,Last Modified Date,Size (KB),Commits,Contributors,Top Language" > repo_activity.csv
...
for REPO in $REPOS; do
  ...
  # Skip repositories with size less than 10KB (likely empty)
  if [ "$REPO_SIZE" -lt 10 ]; then
    echo "Skipping repository with size less than 10KB: $REPO"
    continue
  fi
  ...
  echo "Repository: $REPO - Created on: $CREATION_DATE - Last Modified: $LAST_MODIFIED_DATE - Size: $REPO_SIZE KB - Commits: $COMMIT_COUNT - Contributors: $CONTRIBUTOR_COUNT - Top Language: $TOP_LANGUAGE"

  # Append to CSV file
  echo "$REPO,$CREATION_DATE,$LAST_MODIFIED_DATE,$REPO_SIZE,$COMMIT_COUNT,$CONTRIBUTOR_COUNT,$TOP_LANGUAGE" >> repo_activity.csv
done
