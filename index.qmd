...
chart = {
  ...
  .range(["#440154FF", "#21918c", "#90d743"]);  // Adjusted Viridis colors with more green

  // Scale for node radius based on the number of commits
  const radiusScale = d3.scaleLog()
    .domain([1, 5000])  // Adjust the domain to your data range
    .range([1, 17]);      // Adjust the range for the circle radius
  ...
  function createNodes(scan) {
    ...
    const node = svg.selectAll(".node")
      ...
      .attr("fill", (d) => topRepoCommits.has(d.commits) ? "yellow" : colorScale(d.language))
      .attr("opacity", (d) => topRepoCommits.has(d.commits) ? 1 : 0.7)
      .attr("stroke", (d) => topRepoCommits.has(d.commits) ? "black" : null)  // Add black stroke for top 5
      .attr("stroke-width", (d) => topRepoCommits.has(d.commits) ? 3 : 0);
    ...
    d3.select(this)
      .attr("fill", "yellow")  // Change the fill color to red on mouseover
      .attr("stroke", "black")  // Add black border
      .attr("stroke-width", 2.5);  // Set the border width
  })
    .on("mouseout", function(event, d) {
      d3.select(this)
        .attr("fill", (d) => topRepoCommits.has(d.commits) ? "yellow" : colorScale(d.language))  // Reset the fill color
